...
const Marke = () => {
    const { marks, isLoading, addMark, updateMark, deleteMark } = useMarke()
    ...
    const filteredMarks = marks.filter(mark => {
        return mark.name?.toLowerCase().includes(searchTerm.toLowerCase())
    })
    ...
    const handleSubmit = async (e) => {
        ...
        let result
        if (editingId) {
            result = await updateMark(editingId, formData.name)
        } else {
            result = await addMark(formData.name)
        }
        ...
        if (result.success) {
            showToast(
                editingId ? 'Mark updated successfully!' : 'Mark added successfully!',
                'success'
            )
            closeModal()
        } else {
    ...
    const confirmDelete = async () => {
        setModalLoading(true)
        const result = await deleteMark(selectedMark.id)
        setModalLoading(false)
        ...
        if (result.success) {
            ...
            showToast(result.error || 'Failed to delete mark!', 'error')
        }
    }
    ...
    return (
        <div className="space-y-6 p-6 bg-slate-50 dark:bg-slate-900 min-h-screen">
            {/* Header */}
            <div>
                <h1 className="text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-red-600">Marke Management</h1>
            </div>
            ...
            {isLoading && marks.length === 0 ? (
                ...
                <div className="bg-white dark:bg-slate-800 rounded-xl shadow-md border border-slate-200 dark:border-slate-700">
                    <EmptyState
                        ...
                        description={searchTerm ? 'Try adjusting your search.' : 'Get started by adding your first mark.'}
                        action={!searchTerm ? openAddModal : undefined}
                        actionLabel="Add Mark"
                    />
                </div>
            ) : (
                        ...
                        <table className="w-full">
                            ...
                            <tbody className="divide-y divide-slate-200 dark:divide-slate-700">
                                {filteredMarks.map(mark => (
                                    <tr key={mark.id} className="hover:bg-blue-50 dark:hover:bg-slate-700/50 transition-colors">
                                        <td className="px-6 py-4 text-sm font-semibold text-slate-900 dark:text-white">{mark.name}</td>
            ...
            <Modal
                isOpen={showModal}
                title={editingId ? 'Edit Mark' : 'Add Mark'}
                onClose={closeModal}
                size="lg"
            ...
            <ConfirmModal
                ...
                title="Delete Mark"
                message={`Are you sure you want to delete ${selectedMark?.name}? This action cannot be undone.`}
